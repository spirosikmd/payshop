...
mod_order = Blueprint('order', __name__, url_prefix='/orders')


@mod_order.route('/', methods=['POST'])
def orders():
    orders_url = '{shopping_service_base_url}/orders'.format(
...
def order(order_uid):
    order_url = '{shopping_service_base_url}/orders/{order_uid}'.format(
        shopping_service_base_url=app.config['SHOPPING_SERVICE_BASE_URL'],
        order_uid=order_uid)
    headers = {'content-type': 'application/json'}
